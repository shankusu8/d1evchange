...
(function (e, jQuery) {
    var n = e.document,
        ...
        init = {
            version: 20140530,
            DEBUG: false,
            DOMAIN: 'http://www.d1ev.com/',
            EMBED_STYLESHEET: "statics/js/comment/css/embed3.css?version=" + this.version,
            GET_JSONP: "index.php?g=Comments&m=Index&a=json3",
            POST_JSONP: "index.php?g=Comments&m=Index&a=add3",
            ...
            id: commentsQuery.id,
			article_title: commentsQuery.title,
			article_url: commentsQuery.url,
			//thumb: commentsQuery.thumb,
            //评论设置
            config: {
                'guest': 1, //是否运行游客评论
                'code': 0, //验证码
            ...
            binds: function () {
                ...
                if (init.config.noallow) {
                    ...
                    jQuery('a.ds-post-reply').bind('click', function () {
						...
						var dangqian = reply.data('dangqian');
						$('.ds-post .ds-replybox').hide();
                        //再次点击回复按钮打开关闭回复框
//						jQuery(document).on("click",".ds-post-reply",function(){
//							
//							var obj=$(this).parent().parent().find("div").eq(2);
//							if($(obj).is(":visible")){
//							  $(obj).hide();
//							}else{
//							  $(obj).show();
...
//						$(".ds-replybox").hide();
						$(this).click(function(){
							var obj2014 = $(this).parent().parent().find("div").eq(2);
//							if($(".ds-replybox").css("display") == 'block' ){
//									$(".ds-replybox").hide();
//								}
							obj2014.toggle()
						})
//						
						...
						replyactive.children("form").find('input[name="dangqian"]').attr('value', dangqian);
                        tool.localStorages();
            ...
            ajaxButton: function () {
                ...
                if (ajaxForm_list.length) {
                    jQuery('button.ds-post-button,a.ds-post-link').on('click', function (es) {
                        es.preventDefault();
                        var btn = jQuery(this),
						...
						*/
                        ...
                        if(!init.users.user_id){
                            $('.ws-shadow').show();
                            $('.ws-loginModel').show();
							sessionStorage.setItem("textareaContent", $('.J_CmFormField').val());
                            init.LOCK = false;
                            return false;
                        }
                        ...
                        form.ajaxSubmit({
                            ...
                            success: function (data, statusText, xhr, $form) {
                                ...
                                if (data.status > 0) {
                                    ...
                                    jQuery('textarea[name="content"]').val('')
                                    //alert(data.info);
                                    ws.ws_alert($('.ws_alert'),data.info);
                                }else{
                                    btn.removeProp('disabled').removeClass('disabled');
                                    //alert(data.info);
                                    ws.ws_alert($('.ws_alert'),data.info);
        ...
        emote = {
            ...
            htmls:function(){
                ...
                var emote = '';
                var strhtml = '<div id="ds-smilies-tooltip" style="width: 485px;">\
        ...
        addModel = {
            ...
            commentsInfo: function (thread) {
                ...
                thread.append('<div class="ds-comments-info">\
                                <span id="span_new">最新评论</span> \
                               </div>');
				
				//显示评论数
                jQuery('.ds-highlight').html(init.cursor.total2);
                //加载评论
            ...
            comments: function (thread) {
				if(sessionStorage.getItem("textareaContent")){
					var textarea22=sessionStorage.getItem("textareaContent");
					$('.J_CmFormField').val(sessionStorage.getItem("textareaContent"));
				}		
                var post;
                ...
                jQuery.each(init.response, function (i, rs) {
					if(rs.user_id != 0){
						homeurl = 'href="http://www.d1ev.com/home/' + rs.user_id + '" target="_blank"';
						//mouse = 'onmouseover="showcard('+ rs.user_id +');"';
					}else{
						homeurl = 'href="javascript:;"';
						//mouse = 'onmouseover="nouser();"';
					}
                    var comentId = rs.id;
                    if (rs.content) {
						if(rs.good == 0){
							...
							li_class = 'ds-post jp_pl_bg';
						}
                        post.append('<li class="'+li_class+'">\
                                      <div class="ds-post-self">\
                                          <div class="ds-avatar">\
                                            <a class="ds-avatar2" rel="nofollow author" ' + homeurl + ' data-userid="' + rs.user_id + '"><img src="' + tool.getAvatar(rs.user_id, rs.author_email) + '" onerror="this.src=\'' + init.DOMAIN + 'statics/images/member/nophoto.gif\'"></a>\
                                          </div>\
                                          <div class="ds-comment-body">\
                                            <div class="ds-comment-header"><a class="ds-user-name ds-highlight" ' + homeurl + ' rel="nofollow" data-userid="' + rs.user_id + '">' + rs.author + '</a> <span class="ds-time" title="' + tool.getYearsMonthDay(rs.date * 1000,"yyyy-MM-dd hh:mm:ss") + '">' + tool.getTimeBefore(rs.date * 1000) + '</span> \
                                        </div>\
                                            <p>' + rs.content + '</p>\
                                            <div class="ds-comment-footer ds-comment-actions"> \
                                                <span class="ws-author_area">' + rs.author_area +'</span>\
                    ...
                    jQuery.each(json.child, function (i, rs) {
                        ...
                        if (rs.display == 'none') {
                            ...
                            strHtml += '\
                                  <li class="'+li_class+'">\
                                    <div class="ds-post-self">\
                                      <div class="ds-avatar"><a class="ds-avatar2" rel="nofollow author" ' + homeurl + ' data-userid="' + rs.user_id + '"><img src="' + tool.getAvatar(rs.user_id, rs.author_email) + '"></a></div>\
                                      <div class="ds-comment-body">\
                                        <div class="ds-comment-header">\
                                            <a class="ds-user-name ds-highlight" data-qqt-account="" ' + homeurl + ' rel="nofollow" data-userid="' + rs.user_id + '" data-userid="' + rs.user_id + '">' + rs.author + '</a> <span class="ds-time" title="' + tool.getYearsMonthDay(rs.date * 1000,"yyyy-MM-dd hh:mm:ss") + '">' + tool.getTimeBefore(rs.date * 1000) + '</span> \
                                        </div>\
                                        <p>' + rs.content + '</p>\
                                        <div class="ds-comment-footer ds-comment-actions"> \
                                            <span class="ws-author_area">' + rs.author_area +'</span>\
                                            <a class="ds-post-reply" href="javascript:void(0);" data-dangqian="' + comentId + '" data-comentid="' + json.id + '"><span class="ds-ui-icon"></span>回复</a> \
                                            <a class="ds-post-likes" style="display:none;" href="javascript:void(0);" data-comentid="' + comentId + '"><span class="ds-ui-icon"></span>顶</a> \
                                        </div>\
                        ...
                        if (data.status) {
                            ...
                            if (data.data.response) {
                                jQuery.each(data.data.response, function (i, rs) {
                                    ...
                                    strHtml += '<li class="ds-post">\
                                    <div class="ds-post-self">\
                                      <div class="ds-avatar"><a rel="nofollow author" href="javascript:;"><img src="' + tool.getAvatar(rs.user_id, rs.author_email) + '"></a></div>\
                                      <div class="ds-comment-body">\
                                        <div class="ds-comment-header">\
            ...
            replybox: function () {
                return '<form class="ds_form_post" method="post">\
                            ...
                            <a class="ds-avatar" href="javascript:;;"><img src="' + init.users.avatar + '" onerror="this.src=\'' + init.DOMAIN + 'statics/images/member/nophoto.gif\'"></a>\
                                <input  type="hidden" name="comment_catid" value="' + init.catid + '" />\
                                <input  type="hidden" name="comment_id" value="' + init.id + '" />\
								<input  type="hidden" name="article_title" value="' + init.article_title + '" />\
								<input  type="hidden" name="article_url" value="' + init.article_url + '" />\
                                <input  type="hidden" name="parent" value="" />\
								<input  type="hidden" name="dangqian" value="" />\
            ...
            newsCommentBox: function (thread) {
				...
				if (init.users.qq_bind) {
                    qq_bind = '<input name="commSync[]" type="checkbox" value="qq" id="chk1" /><span id="chk1_img_active"  title="同时分享到腾讯微博"></span>';
                }else{
					qq_bind = '<input type="checkbox" value="qq" class="noqq" id="chk1" /><span id="chk1_img" title="同时分享到腾讯微博"></span>';
				}
				if (init.users.sina_bind) {
                    sina_bind = '<input name="commSync[]" type="checkbox" value="sina_weibo" id="chk2" /><div id="chk2_img_active"  title="同时分享到新浪微博"></div>';
                }else{
					sina_bind = '<input type="checkbox" value="sina_weibo" class="nosina" id="chk2" /><div id="chk2_img"  title="同时分享到新浪微博"></div>';
				}
            ...
            getUser: function () {
                var strHtml = '';
                var nichengHtml = '<input maxlength="6" name="author" placeholder="用户名" value="' + init.users.username + '"/>';
                ...
                if(init.config.guest == 0 && init.users.user_id < 1){
                    strHtml = '<a class="ds-post-link" href="javascript:;">请登录后操作！</a>';
                }
        ...
        tool = {
            ...
            getAvatar: function (uid, email) {
                return init.DOMAIN + 'api.php?m=avatar&uid=' + uid + '&size=' + 45;
            },
            //保存用户输入
