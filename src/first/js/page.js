...
$(function () {
    ...
    $(".ws-like").click(function(){
        ...
        $.post("http://www.d1ev.com/index.php?g=Member&m=favorite&a=zan&type=zan", {newid:$('body').attr('data-newid'),catid:$('body').attr('data-catid'),title:$('body').attr('data-title'),tags:$('body').attr('data-tags')},
            function(data){
                console.log(data);
                //alert(data);
                var $modal=$('.ws_alert');
                if($.trim(data)==200){
                    ...
                    $(".ws-like").css('color','#f60');
                    //$modal.find('.am-modal-bd').html('您的点赞支持是作者最大的东西');
                    ws.ws_alert($modal,'您的赞同是对作者最大的支持');
                    //console.log(numb);
